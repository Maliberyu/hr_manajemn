...
@section('page-subtitle', 'Kelola proses perekrutan karyawan baru')
...
<form method="GET" class="flex flex-wrap gap-2 mb-5">
    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari posisi..."
    ...
    <a href="{{ route('rekrutmen.index') }}" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition">Reset</a>
    <div class="ml-auto">
        <a href="{{ route('rekrutmen.create') }}"
           class="inline-flex items-center gap-1.5 px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition">
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <span class="text-xs text-gray-400">{{ $lowongan->total() }} lowongan</span>
    </div>
    ...
    <div class="flex flex-col items-center gap-2 py-14 text-gray-400">
        ...
        <a href="{{ route('rekrutmen.create') }}" class="text-xs text-blue-600 hover:underline">Buka lowongan baru</a>
    </div>
    @else
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                @foreach($lowongan as $rek)
                @php
                    $badgeColor = match($rek->status) {
                        'buka'           => 'green',
                        'proses_seleksi' => 'blue',
                        'tutup'          => 'gray',
                        'dibatalkan'     => 'red',
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    <td class="px-4 py-3 font-medium text-gray-800">{{ $rek->posisi }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $rek->departemen?->nama ?? '-' }}</td>
                    <td class="px-4 py-3 text-center font-semibold text-gray-700">{{ $rek->jumlah_dibutuhkan }}</td>
                    ...
                    <td class="px-4 py-3 text-xs text-gray-500">
                        {{ $rek->tanggal_tutup?->translatedFormat('d M Y') ?? '-' }}
                    </td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-0.5 text-xs rounded-full font-medium text-{{ $badgeColor }}-700 bg-{{ $badgeColor }}-50">
                            {{ ucfirst(str_replace('_', ' ', $rek->status)) }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-xs text-gray-500">{{ $rek->dibuatOleh?->nama ?? '-' }}</td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-1.5">
                            <a href="{{ route('rekrutmen.show', $rek) }}"
                               class="px-2.5 py-1 text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg transition">Lihat</a>
                            <a href="{{ route('rekrutmen.edit', $rek) }}"
                               class="px-2.5 py-1 text-xs bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg transition">Edit</a>
                        </div>
                    </td>
                </tr>
                @endforeach
