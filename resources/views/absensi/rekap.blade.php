    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4"
        ...
        <form method="GET" action="{{ route('absensi.rekap') }}" id="filterForm">
            ...
            <div class="flex flex-wrap gap-3 items-end">
                ...
                {{-- Generate Rekap --}}
                <div class="ml-auto">
                    <form method="POST" action="{{ route('absensi.rekap.generate') }}"
                          onsubmit="return confirm('Generate rekap absensi untuk {{ \Carbon\Carbon::create($tahun, $bulan)->translatedFormat('F Y') }}?\n\nData rekap_absensi bulan ini akan dibuat/diperbarui.')">
                        @csrf
                        <input type="hidden" name="bulan" value="{{ $bulan }}">
                        <input type="hidden" name="tahun" value="{{ $tahun }}">
                        <button type="submit"
                                class="px-4 py-2 text-sm bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold transition flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                      d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                            </svg>
                            Generate Rekap {{ \Carbon\Carbon::create($tahun, $bulan)->translatedFormat('M Y') }}
                        </button>
                    </form>
                </div>
            </div>
